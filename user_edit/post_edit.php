...
<body>
	...
	<?php
			...
			if($_REQUEST['post'] == ""){
				...
				try{
					$sql2 = 'update users set post = ? where user_id = ?';
					//sql文の組み立て
					Dao::db()->mod_exec($sql2,array($_REQUEST['post'],$_SESSION['login']));
					//$stmt = $dbh->prepare($sql2);
					//PDOのファンクションprepare()で準備をする

					//$stmt->bindParam(1,$_REQUEST['post']);
					//上記のsql文の？を埋める
...
?>
	...
	<center>
		...
		<div>新しい役職名</div>
		<div>
		<input type="text" name="post" value="<?php echo $users['data']['post']?>">
		</div>
		<br>
