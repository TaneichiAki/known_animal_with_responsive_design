...
<body>
	...
	<?php
			...
			if($_REQUEST['lastname'] == ""){
				...
				try{
					$sql2 = 'update users set last_name = ? where user_id = ?';
					//sql文の組み立て
					Dao::db()->mod_exec($sql2,array($_REQUEST['lastname'],$_SESSION['login']));
					//$stmt = $dbh->prepare($sql2);
					//PDOのファンクションprepare()で準備をする

					//$stmt->bindParam(1,$_REQUEST['lastname']);
					//上記のsql文の？を埋める
...
?>
	...
	<center>
		...
		<div>新しい姓	</div>
		<div>
		<input type="text" name="lastname" value="<?php echo $users['data']['last_name']?>">
		</div>
		<br>
